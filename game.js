...
var vecmath = require('./vecmath').vecmath;
...
function ball (Pos, Vel, Grav, R, timeboost, TableP, TableS, B1P, B1S, B2P, B2S)
{
    ...
    self.Move = function() {
        var norm = new vec(0, 0, 1);//new THREE.Vector3(0, 0, 1);
            ...
            && self.Position.z < self.TablePos.z && self.Position.z > self.TablePos.z - self.TableSize.z)
        {
            ...
            self.StartVelocity = new vec(self.Velocity.x, -(self.Velocity.y + 0.01), self.Velocity.z);
        ...
        var g = new vec(0, -self.Gravity, 0);
